# Saltation_Observer
Work in progress for Hybrid Observer showing the impact of the saltation matrices on a linear observer. We show the importance of the transversality hypothesis alongside the contractivness of two matrices : $\mathcal{M}_{\rm before}$ and $\mathcal{M}_{\rm after}$
...
- $L_c = [0.1, 0.25]^\top, L_d = [0.1, 0.1]^\top, K = [0; 0]^\top$ show what happens when only $\mathcal{M}_{\rm before}$ is contracting and not $\mathcal{M}_{\rm after}$. Observer initialized at $\hat{x}_0 = (1-6.10^{-4})x_0$.
